                          ...
                          >
                            <div>
                              <div className="flex items-center justify-between gap-3">
                                <p className="truncate text-[11px] font-semibold uppercase tracking-[0.2em] opacity-80">
                                  {OTA_LABELS[reservation.otaSource]}
                                </p>
                                <span className="ml-2 shrink-0 text-[11px] opacity-85">{STATUS_LABELS[reservation.status]}</span>
                              </div>
                              <p className="mt-3 truncate text-sm font-semibold">{reservation.customer.name}</p>
                              <p className="mt-1 truncate text-xs opacity-90">{formatCurrency(reservation.amount, reservation.currency)}</p>
                            </div>
                            <div className="flex items-center justify-between gap-2 text-[11px] opacity-85">
                              <span className="truncate">{reservation.channelReference}</span>
                              <span>
                                {formatDateTimeLabel(reservation.checkIn)} → {formatDateTimeLabel(reservation.checkOut)}
                              </span>
                            </div>
                          </motion.button>
                        );
