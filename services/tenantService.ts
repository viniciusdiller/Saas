...
import { DEMO_TENANT_ID, demoExpenses, demoReservations, demoRooms } from '@/services/demoData';
...
function shouldUseMock(error: unknown) {
  ...
  return error instanceof Error && error.message.includes('DATABASE_UNAVAILABLE');
}
...
function mapReservation(reservation: InstanceType<ReturnType<typeof getDb>['Reservation']>, roomLocalRoomId: string): Reservation {
  return {
...
function getMockRooms(tenantId: number) {
  return tenantId === DEMO_TENANT_ID ? structuredClone(demoRooms) : [];
}

function getMockReservations(tenantId: number) {
  return tenantId === DEMO_TENANT_ID ? structuredClone(demoReservations) : [];
}

function getMockExpenses(tenantId: number) {
  return tenantId === DEMO_TENANT_ID ? structuredClone(demoExpenses) : [];
}
...
export async function getReservations(tenantId: number): Promise<Reservation[]> {
  try {
    ...
    if (shouldUseMock(error)) {
      return getMockReservations(tenantId);
    }
    throw error;
  }
}
...
export async function updateReservation(tenantId: number, updatedReservation: Reservation): Promise<Reservation> {
  try {
    ...
    if (shouldUseMock(error) && tenantId === DEMO_TENANT_ID) {
      const index = demoReservations.findIndex((reservation) => reservation.id === updatedReservation.id);
      if (index >= 0) {
        demoReservations[index] = structuredClone(updatedReservation);
      }
...
export async function getExpenses(tenantId: number): Promise<Expense[]> {
  try {
    ...
    if (shouldUseMock(error)) {
      return getMockExpenses(tenantId);
    }
    throw error;
  }
}
